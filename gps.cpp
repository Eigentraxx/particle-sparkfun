 ...
 */
void setupGpsModule(){
   Wire.begin(); // i2c for sparkfun gps module
...
/**
 * polled in main loop and in httpCmd
 */
  void getGpsData(){
...
Particle.publish(PUBLISH_EVENT_NAME, output, PRIVATE); // publish doc above then webhook using {{guid}} to push to firebase db
...
/*
  used in setup() to generate guid. used internally no need to be all that secure just unique.
  todo improve random seed.
 */
void guidFunc(){
